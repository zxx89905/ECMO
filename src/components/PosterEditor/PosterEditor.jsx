...
const TrackBtnWrap = styled.div`
    display: flex;
    gap: 10px;
    margin-top: 12px;
`
const TrackFuncBtn = styled.button`
...
function PosterEditor({ albumID, handleClickBack, posterWidth = 2480, posterHeight = 3508, posterRatio = "a4" }) {
    ...
    const [showArtistSignature, setShowArtistSignature] = useState(false);
    ...
    const removeAllParentheses = () => {
        const res = tracklist.replace(/\s*\(.*?\)/g, '');
        setTracklist(res);
    };
    const removeAllBrackets = () => {
        const res = tracklist.replace(/\s*\[.*?\]/g, '');
        setTracklist(res);
    };
                            ...
                            {activeTab === 'information' ? (
                                <EditorSettings>
                                    ...
                                    <NormalInput 
                                        title="曲目上下偏移"
                                        value={trackOffsetY} 
                                        onChange={(e) => setTrackOffsetY(e.target.value)}
                                    />
                                    ...
                                    {showArtistSignature && (
                                        <>
                                            ...
                                            <NormalInput
                                                title="签名左右偏移"
                                                value={signatureHorizontalPosition}
                                                onChange={(e) => setSignatureHorizontalPosition(e.target.value)}
                                            />
                                            <NormalInput
                                                title="签名上下偏移"
                                                value={signatureVerticalPosition}
                                                onChange={(e) => setSignatureVerticalPosition(e.target.value)}
                                            />
                                        </>
                                    )}
                                ...
                                <TracklistContainer>
                                    ...
                                    />
                                    <TrackBtnWrap>
                                        <TrackFuncBtn onClick={removeAllParentheses}>移除 () 及内容</TrackFuncBtn>
                                        <TrackFuncBtn onClick={removeAllBrackets}>移除 [] 及内容</TrackFuncBtn>
                                    </TrackBtnWrap>
                                </TracklistContainer>
                            )}
                            <DivButtons>
